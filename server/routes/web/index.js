module.exports = app => {
  ...
  router.get('/news/init', async (req, res) => {
    const parent = await Category.findOne({
      name: '新闻分类' // 注意这个数据要去后台录入 这里写的是前端写死的 不是动态录入的
    })
  ...
  router.get('/news/list', async (req, res) => {
    ...
    const cats = await Category.aggregate([ // aggregate（mongoose 里面的聚合查询）
      { $match: { parent: parent._id } }, // 找到parent
      {
        $lookup: {
          from: 'articles', // 集合名 mongoose.model('Article', schema, 'articles') 'articles'在model里面是省略的
          localField: '_id',
